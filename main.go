...
func main() {
	...
	r.Handle("/api/user/multipleSections", middlewares.AuthenticateClaims(http.HandlerFunc(handlers.MultipleSectionsUser))).Methods("POST")
	r.Handle("/api/user/addSection", middlewares.AuthenticateClaims(http.HandlerFunc(handlers.AddSectionUser))).Methods("POST")
	r.Handle("/api/user/delSection", middlewares.Authenticate(http.HandlerFunc(handlers.DelSectionUser))).Methods("POST")
	r.Handle("/api/user/addContextItem", middlewares.AuthenticateClaims(http.HandlerFunc(handlers.AddContextItem))).Methods("POST")
	r.Handle("/api/user/delContextItem", middlewares.AuthenticateClaims(http.HandlerFunc(handlers.DelContextItem))).Methods("POST")
	r.Handle("/api/user/updateJSCode", middlewares.AuthenticateClaims(http.HandlerFunc(handlers.UpdateJSCode))).Methods("POST")
